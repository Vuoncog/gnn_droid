...
#string = 'r\"tmp9561\/r2\"'
    ...
    def get_so_cfg_of_apk(self):
        # 1. Luôn luôn tạo thư mục native (dù rỗng) để tránh lỗi tiến trình sau
        if not os.path.exists(self.so_cfg_path):
            os.mkdir(self.so_cfg_path)

        # 2. Sau đó mới xử lý file .so nếu có
        if os.path.isdir(self.so_path):
